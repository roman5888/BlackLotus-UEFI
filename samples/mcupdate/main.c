...
typedef EFI_STATUS(EFIAPI* BOOT_ENTRY)(PVOID imageHandle, PVOID systemTable, BLP_ARCH_SWITCH_CONTEXT fpBlpArchSwitchContext, print_entry print, PVOID printSystemTable);
...
DWORD McUpdateEntry(PVOID* functionTableOut, PVOID* functionTableIn)
{
	...
	if (efiNtHeaders->OptionalHeader.AddressOfEntryPoint)
	{
		...
		*/
		...
		entry(efiImageHandle, efiSystemTable, fpBlpArchSwitchContext, print, printSystemTable);
	}
